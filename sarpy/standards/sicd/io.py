...
import sarpy.standards.general.nitf_elements.des
import sarpy.standards.general.nitf_elements.image
import sarpy.standards.general.nitf_elements.nitf_head
import sarpy.standards.general.nitf_elements.security
...
SCHEMA_DIR = importlib.resources.files("sarpy.standards.sicd.schemas")
...
def _interpolate_corner_points_string(entry, rows, cols, icp):
    ...
    if icp.shape[1] == 2:
        ...
        icp_new[:, :2] = icp
        icp = icp_new
    icp_ecf = sarpy.standards.geocoords.geodetic_to_ecf(icp)
    ...
    for row, col in pattern:
        ...
        pt_latlong = sarpy.standards.geocoords.ecf_to_geodetic(pt_array)[:2]
...
class SicdNitfSecurityFields:
    ...
    def from_security_tags(
        cls, security: sarpy.standards.general.nitf_elements.security.NITFSecurityTags
    ) -> Self:
    ...
    def as_security_tags(
        self,
    ) -> sarpy.standards.general.nitf_elements.security.NITFSecurityTags:
        """Construct a NITFSecurityTags object"""
        return sarpy.standards.general.nitf_elements.security.NITFSecurityTags(
            CLAS=self.clas,
            CLSY=self.clsy,
            CODE=self.code,
            CTLH=self.ctlh,
...
class SicdNitfHeaderFields:
    ...
    @classmethod
    def from_header(cls, file_header: sarpy.standards.general.nitf.NITFHeader) -> Self:
...
class SicdNitfImageSegmentFields:
    ...
    @classmethod
    def from_header(
        cls, image_header: sarpy.standards.general.nitf.ImageSegmentHeader
    ) -> Self:
...
class SicdNitfDESegmentFields:
    ...
    @classmethod
    def from_header(
        cls, de_header: sarpy.standards.general.nitf.DataExtensionHeader
    ) -> Self:
...
class SicdNitfReader:
    ...
    def __init__(self, file: BinaryIO | str | os.PathLike):
        ...
        nitf_details = sarpy.standards.general.nitf.NITFDetails(self._file_object)
        ...
        self._nitf_reader = sarpy.standards.general.nitf.NITFReader(
            nitf_details=nitf_details,
            image_segment_collections=image_segment_collections,
        )
        des_header = sarpy.standards.general.nitf.DataExtensionHeader.from_bytes(
...
def _create_des_manager(sicd_xmltree, des_fields):
    ...
    deshead = sarpy.standards.general.nitf_elements.des.DataExtensionHeader(
        Security=des_fields.security.as_security_tags(),
        UserHeader=sarpy.standards.general.nitf_elements.des.XMLDESSubheader(
    ...
    sicd_des = sarpy.standards.general.nitf.DESSubheaderManager(
...
class SicdNitfWriter:
    ...
    def __init__(self, file, nitf_plan: SicdNitfPlan):
        ...
        now_dt = datetime.datetime.now(datetime.timezone.utc)
        header = sarpy.standards.general.nitf_elements.nitf_head.NITFHeader(
            CLEVEL=3,
        ...
        for i, entry in enumerate(image_segment_limits):
            ...
            subhead = sarpy.standards.general.nitf_elements.image.ImageSegmentHeader(
                ...
                Comments=sarpy.standards.general.nitf_elements.image.ImageComments(
                    [
                        sarpy.standards.general.nitf_elements.image.ImageComment(
                            COMMENT=comment
                        )
                ...
                Bands=sarpy.standards.general.nitf_elements.image.ImageBands(
                    values=[
                        sarpy.standards.general.nitf_elements.image.ImageBand(
                            ISUBCAT=entry
                        )
            ...
            image_managers.append(
                sarpy.standards.general.nitf.ImageSubheaderManager(subhead)
            )

        sicd_des = _create_des_manager(sicd_xmltree, self._nitf_plan.des_fields)

        sicd_details = sarpy.standards.general.nitf.NITFWritingDetails(
            header,
        ...
        self._nitf_writer = sarpy.standards.general.nitf.NITFWriter(
    ...
    def write_image(self, array: npt.NDArray, start: None | tuple[int, int] = None):
        ...
        xml_helper = sarpy.standards.sicd.xml.XmlHelper(self._nitf_plan.sicd_xmltree)
        rows = xml_helper.load("./{*}ImageData/{*}NumRows")
        cols = xml_helper.load("./{*}ImageData/{*}NumCols")
