...
import sarpy.standards.xml as ssxml
...
def compute_scp_coa(sicd_xmltree: lxml.etree.ElementTree) -> lxml.etree.ElementTree:
    ...
    sicd_versions = list(sarpy.standards.sicd.io.VERSION_INFO)
    ...
    if not pre_1_4 and not params.is_monostatic():
        tx_coa = t_coa - (1 / sarpy.constants.speed_of_light) * np.linalg.norm(
            npp.polyval(t_coa, params.Xmt_Poly) - scp
        )
        tr_coa = t_coa + (1 / sarpy.constants.speed_of_light) * np.linalg.norm(
