...
import sarpy.standards.cphd.io
import sarpy.standards.cphd.xml
...
def test_pvp():
    ...
    elem = sarpy.standards.cphd.xml.PvpType().make_elem("{faux-ns}PvpNode", pvp_dict)
    assert sarpy.standards.cphd.xml.PvpType().parse_elem(elem) == pvp_dict

    with pytest.raises(TypeError):
        pvp_dict = {"Offset": 11, "Size": 1, "dtype": "F8"}
        sarpy.standards.cphd.xml.PvpType().set_elem(elem, pvp_dict)
    ...
    with pytest.raises(ValueError):
        ...
        sarpy.standards.cphd.xml.PvpType().set_elem(elem, added_pvp_dict)
...
def test_addedpvp():
    ...
    elem = sarpy.standards.cphd.xml.AddedPvpType().make_elem(
        "{faux-ns}AddedPvpNode", added_pvp_dict
    )
    assert sarpy.standards.cphd.xml.AddedPvpType().parse_elem(elem) == added_pvp_dict
...
def test_transcoders():
    ...
    for xml_file in (DATAPATH / "syntax_only/cphd").glob("*.xml"):
        ...
        schema = lxml.etree.XMLSchema(
            file=sarpy.standards.cphd.io.VERSION_INFO[basis_version]["schema"]
        )
        schema.assertValid(etree)
        xml_helper = sarpy.standards.cphd.xml.XmlHelper(etree)
    ...
    unused_transcoders = sarpy.standards.cphd.xml.TRANSCODERS.keys() - used_transcoders
