...
def test_anglemagnitude():
    ...
    type_obj = sarpy.standards.sidd.xml.AngleMagnitudeType(child_ns="urn:SICommon:1.0")
...
def test_filtercoefficient():
    ...
    type_obj = sarpy.standards.sidd.xml.FilterCoefficientType("rowcol")
    type_obj.set_elem(elem, data)
    assert np.array_equal(type_obj.parse_elem(elem), data)
    type_obj = sarpy.standards.sidd.xml.FilterCoefficientType("phasingpoint")
    type_obj.set_elem(elem, data)
    assert np.array_equal(type_obj.parse_elem(elem), data)
...
def test_intlist():
    ...
    type_obj = sarpy.standards.sidd.xml.IntListType()
    type_obj.set_elem(elem, data)
    assert np.array_equal(type_obj.parse_elem(elem), data)
...
def test_image_corners_type():
    ...
    type_obj = sarpy.standards.sidd.xml.ImageCornersType()
    type_obj.set_elem(elem, data)
    assert np.array_equal(type_obj.parse_elem(elem), data)
...
def test_rangeazimuth():
    ...
    type_obj = sarpy.standards.sidd.xml.RangeAzimuthType(child_ns="urn:SICommon:1.0")
    type_obj.set_elem(elem, data)
    assert np.array_equal(type_obj.parse_elem(elem), data)
...
def test_rowcoldble():
    ...
    type_obj = sarpy.standards.sidd.xml.RowColDblType(child_ns="urn:SICommon:1.0")
    type_obj.set_elem(elem, data)
    assert np.array_equal(type_obj.parse_elem(elem), data)
...
def test_sfapointtype():
    data = [1.1, 1.2, 1.3]
    elem = sarpy.standards.sidd.xml.SfaPointType().make_elem("{ns}SfaPoint", data)
    assert np.array_equal(
        sarpy.standards.sidd.xml.SfaPointType().parse_elem(elem), data
    )
    sarpy.standards.sidd.xml.SfaPointType().set_elem(elem, data[:-1])
    assert np.array_equal(
        sarpy.standards.sidd.xml.SfaPointType().parse_elem(elem), data[:-1]
    )
...
def test_transcoders():
    ...
    schema_dir = importlib.resources.files("sarpy.standards.sidd.schemas.version3")
    ...
    for xml_file in (DATAPATH / "syntax_only/sidd").glob("*.xml"):
        ...
        xml_helper = sarpy.standards.sidd.xml.XmlHelper(etree)
    ...
    unused_transcoders = sarpy.standards.sidd.xml.TRANSCODERS.keys() - used_transcoders
